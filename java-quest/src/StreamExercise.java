...
public class StreamExercise {
  public static void main(String[] args) {
    ...
    System.out.println(numbers.stream().filter(e -> e % 2 == 0).map(e -> e * e)
    ...
    System.out.println(names.stream().filter(e -> e.startsWith("A"))
        .collect(Collectors.toList()));
    ...
    numbers2.stream().max((e1, e2) -> e1 - e2)
        .ifPresent(e -> System.out.println("Max : " + e));
    numbers2.stream().min((e1, e2) -> e1 - e2)
    ...
    System.out.println(words2.stream().filter(e -> e.length() > 3).count());
    // Output: 4
    ...
    System.out.println(employees.stream().filter(e -> e.getSalary() >= 50000)
    ...
    System.out
        .println(persons.stream().collect(Collectors.groupingBy(Person::getAge,
    ...
    System.out.println(staffs.stream()
        .collect(Collectors.groupingBy(e -> e.getGender() == Gender.MALE,
    ...
    System.out.println(numbers4.stream().filter(e -> e >= 10).map(e -> e * 2)
        .collect(Collectors.toList()));
    ...
    System.out.println(names2.stream().map(e -> new Person(e, defaultAge))
        .collect(Collectors.toList()));
    ...
    Deque<String> wordsQueue = words3.stream().map(String::toUpperCase)
    ...
    // Output: [1, 4, 9, 16]
    Integer[] integerArr = numbers5.stream().toArray(Integer[]::new);
    ...
    List<Product> products = List.of(new Product("Book", 10),
        new Product("Pen", 5), new Product("NoteBook", 7));
    System.out.println(products.stream().map(Product::getPrice).reduce(0,
        (e1, e2) -> e1 + e2));
    ...
    List<Worker> workers =
        List.of(new Worker("Alice", "HR"), new Worker("Bob", "IT"),
            new Worker("Charlie", "HR"), new Worker("David", "IT"));
    System.out.println(
        workers.stream().collect(Collectors.groupingBy(Worker::getDepartment,
    ...
    System.out.println(listOfIntegers.stream().flatMap(e -> e.stream())
    ...
    System.out.println(
        fruits.stream().distinct().sorted().collect(Collectors.toList()));
    ...
    List<Children> students2 =
        List.of(new Children("Alice", 45), new Children("Bob", 55),
    ...
    System.out.println(
        ages.stream().filter(s -> s % 3 == 0).findFirst().orElse(null));
    ...
    System.out.println(elements.stream().skip(3).collect(Collectors.toList()));
    ...
    System.out.println(amounts.stream().map(e -> e * 2)
    ...
    System.out
        .println(animals.stream().filter(s -> s.length() > 4).findFirst());

    System.out
        .println(animals2.stream().filter(s -> s.length() > 4).findFirst());
    ...
    System.out
        .println(duplicates.stream().distinct().collect(Collectors.toSet()));
    ...
    Collector<Integer, ?, LinkedHashSet<Integer>> customCollector =
        Collector.of(LinkedHashSet::new, LinkedHashSet::add, combiner,
            Collector.Characteristics.UNORDERED);
    System.out.println(duplicates.stream().collect(customCollector));
    ...
    System.out.println(
        keywords.stream().collect(Collectors.summingInt(String::length)));
  }
